...
async function publishMarkdownFile(filePath, spaceFilter, parentPageId, auth, config) {
  ...
  if (hasMermaidDiagram) {
    ...
    while ((match = mermaidTestRegex.exec(content)) !== null) {
      ...
      const mmdDir = config.mmdDir || 'architecture/current/btms/mmd'
      ...
      if (imagePath) {
        ...
        processedContent = processedContent.replace(
          ...
          `<ac:image-placeholder-viewid="${diagramId}"/>`
        )
        processedContent = processedContent.replace(
  ...
  if (publishedPageId) {
    ...
    while ((match = imageRegex.exec(content)) !== null) {
      ...
      // Skip absolute URLs (http/https) and likec4-exports (handled separately)
      if (imagePath.startsWith('http://') || imagePath.startsWith('https://') || imagePath.startsWith('/likec4-exports/')) {
        continue
      }
    ...
    for (const img of markdownImages) {
      const attachmentData = await imageHandler.uploadImageAttachment(publishedPageId, img.imagePath, auth)
      if (attachmentData) {
        const attachmentInfo = `${attachmentData.attachmentId}|${attachmentData.fileId || ''}|${attachmentData.filename}`
        placeholders.push({
          viewId: img.filename,
