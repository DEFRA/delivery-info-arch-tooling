...
function filterContentForFormat(content, format) {
  if (format === 'confluence') {
    ...
    content = content.replace(
      /<!--\s*PPT_ONLY\s*-->[\s\S]*?<!--\s*\/\s*PPT_ONLY\s*-->\n?/gi,
      ''
    )
    content = content.replace(
      /<!--\s*GITHUB_ONLY\s*-->[\s\S]*?<!--\s*\/\s*GITHUB_ONLY\s*-->\n?/gi,
      ''
    )
    ...
    content = content.replace(/<!--\s*NOT_PPT\s*-->/gi, '')
    content = content.replace(/<!--\s*\/\s*NOT_PPT\s*-->/gi, '')
    ...
    content: [
      ...
      },
      {
        ...
        content: [
          {
            type: 'text',
            text: 'Inline comments will be lost on update, please consider using page comments instead.'
          }
        ]
      }
    ]
...
<p>Inline comments will be lost on update, please consider using page comments instead.</p>
</ac:rich-text-body>
</ac:structured-macro>`
