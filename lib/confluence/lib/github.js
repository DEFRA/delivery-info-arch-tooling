...
function normalizeGitHubPath (filePath) {
...
function getGitHubSourceUrl (filePath, isDirectory = false) {
  ...
  const encodedPath = normalizeGitHubPath(relPath)
    .replace(/^\/+/, '')
    .split('/')
...
/**
 * Build a markdown caption linking a diagram to its source file on GitHub
 * Used to place a "Diagram source" line beneath rendered Mermaid images so
 * readers of the published page can reach the editable .mmd.
 * @param {string} sourcePath - Path to the diagram source file (e.g. a .mmd)
 * @returns {string|null} Markdown caption, or null if no GitHub repo is detected
 */
function getDiagramSourceCaption (sourcePath) {
  const url = getGitHubSourceUrl(sourcePath)
  if (!url) {
    return null
  }
  const fileName = path.basename(normalizeGitHubPath(sourcePath))
  // Italic label then the link: the ADF converter does not carry emphasis across a link
  return `*Diagram source:* [${fileName}](${url})`
}

module.exports = {
  getGitHubSourceUrl,
  getDiagramSourceCaption
}
