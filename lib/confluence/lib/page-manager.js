...
async function addLabelToPage (pageId, labelName, auth) {
  ...
  if (CONFIG.dryRun) {
    console.error(`    🧪 Dry run: would add label '${labelName}' to page ${pageId}`)
    return true
  }
...
async function handlePageStatus (pageId, pageStatus, title, auth) {
  ...
  if (pageStatus === 'archived') {
    if (CONFIG.dryRun) {
      console.error(`    🧪 Dry run: would restore archived page: ${title}`)
      return { usable: true, pageId, title }
    }
  ...
  if (pageStatus === 'trashed') {
    if (CONFIG.dryRun) {
      console.error(`    🧪 Dry run: would permanently delete trashed page: ${title}`)
