...
/**
 * Find the .mmd source file for a Mermaid diagram ID
 * If mmdDir is configured it is checked first; otherwise (or if not found
 * there) the whole repository is searched, excluding node_modules, build and
 * generated. This mirrors `npm run build:mmd`, which renders every .mmd in
 * the repo to generated/diagrams/<basename>.png, so sources can live
 * anywhere (architecture/current/*, architecture/adr/*, architecture/dr/*).
 * @param {string} diagramId - Diagram ID (the .mmd basename)
 * @param {string|null} [mmdDir] - Optional preferred directory for .mmd files
 * @returns {Promise<string|null>} Path to the .mmd file (relative to cwd) or null
 */
async function findMermaidSource(diagramId, mmdDir = null) {
  if (mmdDir) {
    const preferred = path.join(mmdDir, `${diagramId}.mmd`)
    try {
      await fs.access(preferred)
      return preferred
    } catch (e) {
      // Not in the preferred directory, fall through to repo search
    }
  }
  ...
  try {
    ...
    if (glob) {
      ...
      const matches = await globFn(`**/${diagramId}.mmd`, {
        cwd: process.cwd(),
        ignore: ['**/node_modules/**', 'build/**', 'generated/**']
      })
      if (matches && matches.length > 0) {
        // Deterministic choice when several files share a basename
 ...
 * Looks for pre-rendered PNG in generated/diagrams (flat structure) first,
 * then falls back to build/mmd, then tries to render from .mmd source
 ...
 */
async function findMermaidDiagram(diagramId, mmdDir = null, outputDir = 'build/mmd') {
  // First, check generated/diagrams/ for flat PNG (from npm run build:mmd)
  const flatPngPath = path.join('generated', 'diagrams', `${diagramId}.png`)
  try {
    await fs.access(flatPngPath)
    console.error(`    → Found pre-rendered Mermaid diagram: ${flatPngPath}`)
    return flatPngPath
  } catch (e) {
    // Not in generated/diagrams, continue searching
  }

  const mmdFile = await findMermaidSource(diagramId, mmdDir)
...
async function uploadImageAttachment(pageId, imagePath, auth) {
  ...
  if (CONFIG.dryRun) {
    console.error(`    🧪 Dry run: would upload image '${filename}' to page ${pageId}`)
    return null
  }
...
async function replaceImagePlaceholdersAtlas(atlasJson, placeholderData, pageId) {
  ...
  for (const item of placeholderData) {
    ...
    // Optional per-placeholder width (e.g. MermaidDiagram width="900") limits Confluence display size
    const targetWidth = (item.width && item.width > 0) ? item.width : 1600
    let imageHeight = 928
...
function replaceImagePlaceholders(content, placeholderData, format = 'storage') {
  ...
  for (const item of placeholderData) {
    ...
    let imageWidth = item.width ? String(item.width) : '1200'
    const viewLower = viewId.toLowerCase()
    if (!item.width && (viewLower.includes('network') || viewLower.includes('architecture') ||
      viewLower.includes('infrastructure') || viewLower.includes('deployment'))) {
      imageWidth = '1600'
    } else if (!item.width && (viewLower.includes('flow') || viewLower.includes('endtoend') ||
