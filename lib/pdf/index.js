...
const { resolveDiagramTags } = require('./diagrams')
...
async function exportToPdf (options) {
  ...
  // Resolve diagram component tags to images (relative to the markdown file).
  // Serve the repository root so those relative paths, and any others in the
  // document, resolve; md-to-pdf keeps the page URL at the markdown's location.
  const markdown = fs.readFileSync(inputPath, 'utf8')
  const content = resolveDiagramTags(markdown, { inputPath })
  pdfOptions.basedir = process.cwd()

  // Convert markdown to PDF
  const pdf = await mdToPdf({ path: inputPath, content }, pdfOptions)
