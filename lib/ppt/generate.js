...
function parseArgs() {
  ...
  return { inputFile, outputFile, theme, title, author, date, version, cssFile, keepMarp, applyTemplate, templateFile, headingLevel };
}
...
function getBackgroundImagePath(inputFile, rootDir, imageType = 'title', slideSize = '16:9') {
  ...
  const bgImagePath = join(rootDir, 'templates', imageName);

  // Fallback to old name if new one doesn't exist
  if (!existsSync(bgImagePath)) {
    const fallbackPath = join(rootDir, 'templates', 'defra-title-background.png');
    if (existsSync(fallbackPath)) {
      const inputDir = dirname(inputFile);
      const relativePath = relative(inputDir, fallbackPath);
  ...
  if (existsSync(bgImagePath)) {
    // Calculate relative path from input file to templates directory
    const inputDir = dirname(inputFile);
...
function convertMarpToPptx(marpFile, outputFile, theme, rootDir) {
...
function convertMdToPptxViaMarp(inputFile, options) {
  ...
  let pptxOutput = convertMarpToPptx(marpFile, options.outputFile, options.theme, rootDir);
