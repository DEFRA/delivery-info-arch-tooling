...
const { buildPptDiagrams } = require('./build-diagrams');
const { applyPptSlideMarkers } = require('./slide-breaks');
...
function parseArgs() {
  ...
  let buildDiagrams = false; // Build only diagrams referenced by the input markdown
  ...
  return { inputFile, outputFile, theme, title, author, date, version, cssFile, keepMarp, applyTemplate, templateFile, headingLevel, editable, buildDiagrams };
}
...
function convertDiagramComponentToImage(markdown, inputFile, rootDir, componentName, attributeName) {
  const regex = new RegExp(`<${componentName}[^>]*${attributeName}="([^"]+)"[^>]*\\/?>`, 'g');

  return markdown.replace(regex, (match, diagramId) => {
    const imagePath = findDiagramImage(diagramId, inputFile, rootDir);

    if (imagePath) {
      return `![${diagramId}](${imagePath})`;
    }

    console.warn(`⚠️  Warning: Diagram image not found for ${attributeName} "${diagramId}"`);
    console.warn(`   Expected locations:`);
    console.warn(`   - generated/diagrams/${diagramId}.png`);
    console.warn(`   - astro/likec4-exports/${diagramId}.png (legacy)`);
    console.warn(`   - architecture/export/${diagramId}.png`);
    console.warn(`   Run: build-ppt-diagrams "${inputFile}"`);
    console.warn(`   Using placeholder - diagram may not appear in PPT\n`);
    return `![${diagramId}](./images/${diagramId}.png)`;
  });
}

function convertLikeC4ViewToImage(markdown, inputFile, rootDir) {
  return convertDiagramComponentToImage(markdown, inputFile, rootDir, 'LikeC4View', 'viewId');
}

function convertMermaidDiagramToImage(markdown, inputFile, rootDir) {
  return convertDiagramComponentToImage(markdown, inputFile, rootDir, 'MermaidDiagram', 'diagramId');
}

function normalizeListSpacingForPpt(content) {
  // Marp can render "loose" markdown lists (with blank lines between items)
  // as block paragraphs in PPTX, which may split "1." onto its own line.
  // For PPT output, collapse blank lines between adjacent list items.
...
function convertHeadingsToSlideBreaks(content, inputFile, rootDir, headingLevel = 1) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Handle explicit --- slide breaks already in the markdown
    if (!inCodeBlock && line.match(/^---\s*$/)) {
      ...
      // Check if the next slide is a section slide (look ahead)
      ...
      // Add white background directive before the separator for content slides
      if (!isNextSection) {
        result.push('<!-- _backgroundColor: #ffffff -->');
      }
      result.push(line);

      // Reset slide tracking at explicit separators
    ...
    if (line.match(headingPattern)) {
      ...
      for (let j = result.length - 1; j >= 0; j--) {
        if (result[j] && result[j].match(/^---\s*$/)) {
          ...
          // Check a range of lines before the separator for existing directives
          const nearbyLines = result.slice(Math.max(0, j - 4), j);
          const alreadyHasSection = nearbyLines.some(l => l && l.includes('section-slide'));
...
function createMarpFrontmatter(options, frontmatter, rootDir, inputFile) {
  ...
  const finalDescription = frontmatter.description || '';
...
<style>
/* Marp Core v4+ default theme centres via place-content: safe center center */
section {
  display: block;
  place-content: safe start start !important;
  align-content: start !important;
  padding: 50px 78.5px 78.5px 78.5px;
}

section.title-slide,
section.section-slide {
  place-content: safe center center !important;
...
${finalDescription ? `<p class="title-page-sub">${finalDescription}</p>` : ''}
...
function convertToMarp(inputFile, options, rootDir) {
  ...
  // Normalize list spacing to avoid PPTX numbered-list line break issues.
  marpContent = normalizeListSpacingForPpt(marpContent);

  // Headerless slide breaks for diagram-heavy slides (<!-- PPT_SLIDE -->)
  marpContent = applyPptSlideMarkers(marpContent);

  // Convert absolute /docs/... image paths to relative paths
  marpContent = convertAbsoluteImagePaths(marpContent, inputPath, rootDir);

  // Convert diagram components to images
  marpContent = convertLikeC4ViewToImage(marpContent, inputPath, rootDir);
  marpContent = convertMermaidDiagramToImage(marpContent, inputPath, rootDir);
...
function findThemeCSSPath(rootDir) {
  const possiblePaths = [
    join(rootDir, 'templates', 'defra-marp-theme.css'),
    join(libraryTemplatesDir, 'defra-marp-theme.css'),
  ];
...
function convertMarpToPptx(marpFile, outputFile, theme, rootDir, editable = false) {
  ...
  // Pass theme CSS file path so Marp registers it as a proper theme
  // (inline <style> blocks don't get theme-level specificity)
  const themeCSSPath = findThemeCSSPath(rootDir);
  if (themeCSSPath) {
    marpArgs.push('--theme', `"${themeCSSPath}"`);
  } else if (theme) {
...
function convertMdToPptxViaMarp(inputFile, options) {
  ...
  if (options.buildDiagrams) {
    console.log('Building diagrams referenced by PPT content...\n');
    buildPptDiagrams({ inputFile: inputPath });
    console.log('');
  }
...
if (!options.inputFile) {
  ...
  console.error('  --build-diagrams     Build only LikeC4/Mermaid images referenced by the input markdown');
  console.error('\nExample:');
